 ...
 * The contact panel.
 *
 * There is no phone number here, and no "show number" button. Numbers are
 * exchanged only inside a conversation, only after both sides agree — see
 * docs/03-trust-and-safety.md. A reveal button on a public page would make
 * every listing a scrapeable directory of phone numbers, which is the exact
 * failure mode listers on the incumbent portals complain about.
 */
...
function joinedLabel(joinedAt: string): string {
  ...
  // A visibly new account costs an honest lister very little and a scammer a
  // great deal, so it is stated plainly rather than hidden.

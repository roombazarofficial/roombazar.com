...
export const createListingSchema = z.object({
  roomType: z
    ...
    .default("singleroom"),
  postedBy: z.enum(["owner", "tenant", "agent"]).default("owner"),
  localitySlug: z.string().optional().default("general"),
  citySlug: z.string().optional().default("delhi"),

  rentPaise: z.number().int().min(0).max(100_000_000).default(0),
  ...
  */
  ...
  areaSqft: z.number().int().min(1).max(20_000).nullable().default(null),
  ...
  availableFrom: z.string().optional().default(() => new Date().toISOString().slice(0, 10)),
